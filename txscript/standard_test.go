...
// newAddressPubKey returns a new mvcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) mvcutil.Address {
	addr, err := mvcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new mvcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) mvcutil.Address {
	addr, err := mvcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new mvcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) mvcutil.Address {
	addr, err := mvcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the mvcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the mvcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := mvcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := mvcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := mvcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := mvcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := mvcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*mvcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*mvcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*mvcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*mvcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*mvcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
