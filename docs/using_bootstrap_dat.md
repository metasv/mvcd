...
5. [How do I use bootstrap.dat with bsvd?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Bsvd will download the
...
### 5. How do I use bootstrap.dat with bsvd?

bsvd comes with a separate utility named `addblock` which can be used to import
...
1. Stop bsvd if it is already running.  This is required since addblock needs to
   access the database used by bsvd and it will be locked if bsvd is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\metasv\bsvd\addblock" -i C:\Path\To\bootstrap.dat
```
