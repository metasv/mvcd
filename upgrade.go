...
// oldBsvdHomeDir returns the OS specific home directory bsvd used prior to
// version 0.3.3.  This has since been replaced with mvcutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBsvdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "bsvd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".bsvd")
	}
...
// location prior to bsvd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to bsvd
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldBsvdHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bsvd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bsvd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "bsvd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to bsvd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		bsvdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old bsvd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			bsvdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to bsvd as new versions require it.
