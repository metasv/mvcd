...
type memWallet struct {
	...
	addrs map[uint32]mvcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]mvcutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*mvcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          mvcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (mvcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []mvcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt mvcutil.Amount,
	feeRate mvcutil.Amount, change bool) ([]mvcutil.Amount, error) {
	...
	var (
		amtSelected  mvcutil.Amount
		txSize       int
		inputAmounts []mvcutil.Amount
	)
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := mvcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt mvcutil.Amount
	for _, output := range outputs {
		outputAmt += mvcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
	...
	for i, txIn := range tx.TxIn {
		...
		sigScript, err := txscript.SignatureScript(tx, i,
			int64(inputAmounts[i].ToUnit(mvcutil.AmountSatoshi)), utxo.pkScript,
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() mvcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance mvcutil.Amount
...
func keyToAddr(key *bsvec.PrivateKey, net *chaincfg.Params) (mvcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := mvcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
