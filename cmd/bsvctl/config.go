...
var (
	...
	defaultWalletCertFile = filepath.Join(bsvwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for bsvctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(bsvctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default bsvctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(bsvwalletHomeDir, "bsvwallet.conf")
		} else {
			serverConfigPath = filepath.Join(bsvdHomeDir, "bsvd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either bsvd or
// bsvwallet), and extract the RPC user and password from it.
