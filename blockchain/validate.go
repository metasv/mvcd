...
const (
	...
	baseSubsidy = 50 * mvcutil.SatoshiPerBitcoin
...
func IsCoinBase(tx *mvcutil.Tx) bool {
...
func IsFinalizedTransaction(tx *mvcutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *mvcutil.Tx, scriptFlags txscript.ScriptFlags) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > mvcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *mvcutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *mvcutil.Tx, scriptFlags txscript.ScriptFlags) int {
	msgTx := tx.MsgTx()
...
func GetSigOps(tx *mvcutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint, scriptFlags txscript.ScriptFlags) (int, error) {
...
func CountP2SHSigOps(tx *mvcutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint, scriptFlags txscript.ScriptFlags) (int, error) {
...
func checkBlockSanity(block *mvcutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *mvcutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *mvcutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *mvcutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *mvcutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(block *mvcutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *mvcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", mvcutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > mvcutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				mvcutil.Amount(originTxSatoshi),
				mvcutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > mvcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *mvcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *mvcutil.Block) error {
	b.chainLock.Lock()
