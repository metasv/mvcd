...
const (
	...
	baseSubsidy = 50 * bsvutil.SatoshiPerBitcoin
...
func IsCoinBase(tx *bsvutil.Tx) bool {
...
func IsFinalizedTransaction(tx *bsvutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *bsvutil.Tx, scriptFlags txscript.ScriptFlags) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > bsvutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *bsvutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *bsvutil.Tx, scriptFlags txscript.ScriptFlags) int {
	msgTx := tx.MsgTx()
...
func GetSigOps(tx *bsvutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint, scriptFlags txscript.ScriptFlags) (int, error) {
...
func CountP2SHSigOps(tx *bsvutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint, scriptFlags txscript.ScriptFlags) (int, error) {
...
func checkBlockSanity(block *bsvutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
func ExtractCoinbaseHeight(coinbaseTx *bsvutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *bsvutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *bsvutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(block *bsvutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *bsvutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", bsvutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > bsvutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				bsvutil.Amount(originTxSatoshi),
				bsvutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > bsvutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *bsvutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *bsvutil.Block) error {
	b.chainLock.Lock()
